...
function Card({
  ...
  const ref = useReveal<HTMLDivElement>();
  // The outer wrapper owns the scroll-reveal (its `transform` is driven by the
  // `[data-reveal]` CSS). The inner card owns the hover lift on its OWN transform,
  // so the two animations never fight over the same property.
  return (
    <div
      ref={ref}
      data-reveal
    ...
    >
      <div className="flex h-full flex-col rounded-[2rem] md:rounded-[2.8rem] border border-black/[0.055] bg-gradient-to-b from-[#fdfdfe] to-[#f1f1f4] p-[2.4rem] md:p-[3.2rem] shadow-[inset_0_1px_0_rgba(255,255,255,0.7),0_1px_2px_rgba(0,0,0,0.04),0_24px_48px_-30px_rgba(0,0,0,0.18)] transition-[transform,box-shadow] duration-[700ms] ease-[var(--ease-spring-lift)] hover:-translate-y-[6px] hover:shadow-[inset_0_1px_0_rgba(255,255,255,0.7),0_2px_4px_rgba(0,0,0,0.05),0_44px_72px_-32px_rgba(0,0,0,0.28)] motion-reduce:transition-none motion-reduce:hover:translate-y-0">
...
export function Bento({ dict }: { dict: BentoDict }) {
  return (
    ...
    >
      <Heading dict={dict} />
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-[2rem] md:gap-[2.4rem] mt-[5rem] md:mt-[6rem]">
        {cardLayout.map((c, i) => (
